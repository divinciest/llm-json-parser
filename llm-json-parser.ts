...
import { safeStringify } from "./safe_stringify";
...
const { createParser } = require('really-relaxed-json')
...
export function bestMatchObjectAccess(obj, key, caseInsensitive = false, searchInKeys = false) {
    ...
    let result = bestScore <= 0.30 ? obj[bestMatch] : null;
    if (result != null) {
        return result;
    }
...
function prepareSchema(schema, caseInsensitive = false) {
    ...
    let schemaCaseProcessed = null
    let schemaValueProcessed = null
...
export function parseJSON(jsonString, schema,
    ...
    let allResults = []; // here we store all lists of choices that reach the end in DFS

    const LeaftNodesPaths = getLeafPaths(schema) // get all leaf node dot-paths , we can not pick leaf note dot-paths twice 
    ...
    while (stack.length > 0) { // DFS 
        let { currentPath, currentResult, position, forkId } = stack.pop(); // pop from stack our last picked choice
        while (position < pairs.length) {  // continue predicting future line dot-paths 
            ...
            for (let i = 1; i < possiblePaths.length; i++) {
                ...
                stack.push({
                    currentPath: newPath,
                    currentResult: [...currentResult, { path: newPath || "undefined", line, arrayResolvedPath: newPath || "undefined", attr: attr || "" }],
                    position: position + 1,
                    forkId: childForkId
                });
                childForkId++;
    ...
    allResults.forEach(resultObj => {
        ...
        arrayPathsToProcess.forEach(arrayPath => {
            let indexCounter = -1;
            let firstElementPath = null;
